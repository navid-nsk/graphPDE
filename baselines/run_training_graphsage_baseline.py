"""
run_training_graphsage_baseline.py

Train GraphSAGE baseline model for multi-ethnic population prediction.
Uses sampling-based graph aggregation (Hamilton et al. 2017).
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Train GraphSAGE baseline for ethnicity population prediction'
    )
    ...
    parser.add_argument('--experiment_name', type=str, default='graphsage_baseline',
...
def main():
    ...
    # Create model
    print("\n" + "="*80)
    print("CREATING GRAPHSAGE MODEL" if not resume_checkpoint else "CREATING GRAPHSAGE MODEL (will load from checkpoint)")
    print("="*80)
    ...
    print(f"\nModel: GraphSAGE Baseline (Graph Sample and Aggregate)")
    ...
    print(f"  GraphSAGE layers: {args.sage_num_layers} × SAGE (dim={args.sage_hidden_dim}, aggr={args.aggr})")
    print(f"    → Sampling-based aggregation (Hamilton et al. 2017)")
    print(f"    → More scalable than full-neighborhood GCN")
    print(f"  Ethnicity embedding: {n_ethnicities} → {args.ethnicity_embed_dim}")
    print(f"  Predictor: {args.sage_hidden_dim + args.ethnicity_embed_dim} → 1")
