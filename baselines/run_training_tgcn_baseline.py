"""
run_training_tgcn_baseline.py

Train TGCN baseline model for multi-ethnic population prediction.
Combines GCN spatial layers with GRU temporal layers for spatial-temporal modeling.
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Train TGCN baseline for ethnicity population prediction'
    )
    ...
    parser.add_argument('--experiment_name', type=str, default='tgcn_baseline',
...
def main():
    ...
    # Create model
    print("\n" + "="*80)
    print("CREATING TGCN MODEL" if not resume_checkpoint else "CREATING TGCN MODEL (will load from checkpoint)")
    print("="*80)
    ...
    print(f"\nModel: TGCN Baseline (Temporal Graph Convolutional Network)")
    ...
    print(f"  TGCN layers: {args.tgcn_num_layers} × TGCN (dim={args.tgcn_hidden_dim})")
    print(f"    → GCN (spatial) + GRU (temporal)")
    print(f"    → Sequence length: {args.sequence_length}")
    print(f"  Ethnicity embedding: {n_ethnicities} → {args.ethnicity_embed_dim}")
    print(f"  Predictor: {args.tgcn_hidden_dim + args.ethnicity_embed_dim} → 1")
