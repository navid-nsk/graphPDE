"""
run_training_mlp_baseline.py

Train MLP baseline model for multi-ethnic population prediction.
No spatial structure - like XGBoost but with neural network.
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Train MLP baseline for ethnicity population prediction'
    )
    ...
    parser.add_argument('--experiment_name', type=str, default='mlp_baseline',
...
def main():
    ...
    print(f"  Other ethnic features: {n_ethnicities - 1}")
    ...
    # Create model
    print("\n" + "="*80)
    print("CREATING MLP MODEL" if not resume_checkpoint else "CREATING MLP MODEL (will load from checkpoint)")
    print("="*80)
    ...
    print(f"\nModel: MLP Baseline (No Spatial Structure)")
    print(f"  Total parameters: {n_params:,}")
    print(f"  Trainable parameters: {n_trainable:,}")
    print(f"  Model size: {n_params * 4 / 1024 / 1024:.2f} MB (float32)")
    
    n_input = n_census_features + n_ethnicities
    print(f"\nArchitecture:")
    print(f"  Input: {n_input} features (census + other ethnicities)")
    print(f"    - Census features: {n_census_features}")
    print(f"    - Other ethnic features: {n_ethnicities}")
    print(f"  Feature encoder: {n_input} → {args.feature_encoder_dim} → {args.feature_encoder_dim} → {args.feature_encoder_dim // 2}")
    print(f"  Ethnicity embedding: {n_ethnicities} → {args.ethnicity_embed_dim}")
    print(f"  Predictor: {args.feature_encoder_dim // 2 + args.ethnicity_embed_dim} → {args.predictor_hidden_dim} → {args.predictor_hidden_dim // 2} → 1")
    print(f"\nKey characteristic: NO spatial structure (treats samples independently)")
