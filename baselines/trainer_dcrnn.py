...
Trainer for DCRNN baseline - matches GraphPDE trainer interface exactly.
"""
...
class TrainerDCRNN:
    """
    Trainer for DCRNN baseline.
    
    Matches GraphPDE trainer interface for fair comparison.
    """
    def __init__(
        self,
        model,
    ...
    def _prepare_city_mapping(self):
        ...
        print(f"Prepared city mapping:")
        print(f"  Cities: {len(unique_cities)} unique cities")
    ...
    def _initialize_metrics_csv(self):
        ...
        print(f"Metrics will be logged to: {self.metrics_csv_path}")
    ...
    def train_epoch(self):
        ...
        with tqdm(self.train_loader, desc=f"Epoch {self.current_epoch}") as pbar:
            for batch_idx, batch in enumerate(pbar):
                ...
                output = self.model(batch, city_indices, debug_batch=False)
                ...
                if torch.isnan(predictions).any() or torch.isinf(predictions).any():
                    print(f"\n💥 Batch {batch_idx}: NaN/Inf in predictions! Skipping batch...")
                    continue
                ...
                if torch.isnan(loss) or torch.isinf(loss):
                    print(f"\n💥 Batch {batch_idx}: NaN/Inf loss! Skipping...")
                    continue
                
                if loss > 1e8:
                    print(f"\n💥 Batch {batch_idx}: Huge loss {loss.item():.2e}! Skipping...")
                    continue
                ...
                pbar.set_postfix({
                    'loss': f'{loss.item():.2e}',
                    'max_pred': f'{predictions.max().item():.1f}'
                })
    ...
    def train(self, n_epochs, scheduler=None, early_stopping_patience=10, start_epoch=0):
        ...
        if start_epoch >= n_epochs:
            print(f"\n⚠️  Already at epoch {start_epoch}, which is >= target epochs {n_epochs}")
            print("No training needed. To continue training, increase --epochs parameter.")
            print("Training complete - no additional epochs to run.")
            return
...
if __name__ == "__main__":
    print("TrainerDCRNN module loaded successfully!")
