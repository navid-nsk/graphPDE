"""
run_training_deeponet_baseline.py

Train DeepONet baseline model for multi-ethnic population prediction.
...
from model_deeponet_baseline import create_deeponet_model
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Train DeepONet baseline for ethnicity population prediction'
    )
    ...
    parser.add_argument('--branch_hidden_dims', type=int, nargs='+', default=[256, 256, 256],
                        help='Hidden dimensions for branch network')
    parser.add_argument('--trunk_hidden_dims', type=int, nargs='+', default=[128, 128, 128],
                        help='Hidden dimensions for trunk network')
    parser.add_argument('--latent_dim', type=int, default=128,
                        help='Latent dimension (must match for branch & trunk)')
    parser.add_argument('--ethnicity_embed_dim', type=int, default=32,
                        help='Ethnicity embedding dimension')
    parser.add_argument('--dropout', type=float, default=0.1,
                        help='Dropout rate')
    parser.add_argument('--use_spatial_context', action='store_true',
                        help='Use spatial neighborhood aggregation')
    ...
    parser.add_argument('--resume_training_only', action='store_true',
                        help='Only resume training state, not metrics history')
    ...
    parser.add_argument('--experiment_name', type=str, default='deeponet_baseline',
...
def main():
    ...
    # Create model
    print("\n" + "="*80)
    print("CREATING DEEPONET MODEL" if not resume_checkpoint else "CREATING DEEPONET MODEL (will load from checkpoint)")
    print("="*80)
    
    model = create_deeponet_model(
        n_census_features=n_census_features,
        n_ethnicities=n_ethnicities,
        adjacency=graph['adjacency'] if args.use_spatial_context else None,
    ...
    print(f"\nModel: DeepONet (Deep Operator Network)")
    print(f"  Total parameters: {n_params:,}")
    print(f"  Trainable parameters: {n_trainable:,}")
    print(f"  Model size: {n_params * 4 / 1024 / 1024:.2f} MB (float32)")
    
    print(f"\nArchitecture:")
    print(f"  Branch net (spatial context encoder):")
    print(f"    Input: {n_census_features + 1 + 2} (census + aggregated pop + coordinates)")
    print(f"    Hidden layers: {args.branch_hidden_dims}")
    print(f"    Output: {args.latent_dim} (latent space)")
    print(f"  Trunk net (query encoder):")
    print(f"    Input: ethnicity embedding ({args.ethnicity_embed_dim} dim)")
    print(f"    Hidden layers: {args.trunk_hidden_dims}")
    print(f"    Output: {args.latent_dim} (latent space)")
    print(f"  Operator: Inner product of branch & trunk outputs")
    print(f"  Spatial context: {'Enabled (with neighborhood aggregation)' if args.use_spatial_context else 'Disabled'}")
