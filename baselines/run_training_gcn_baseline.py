"""
run_training_gcn_baseline.py

Train GCN baseline model for multi-ethnic population prediction.
Uses standard graph convolution (Kipf & Welling 2017) instead of attention.
...
from model_gcn_baseline import create_gcn_model
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Train GCN baseline for ethnicity population prediction'
    )
    ...
    parser.add_argument('--experiment_name', type=str, default='gcn_baseline',
...
def main():
    ...
    # Create model
    print("\n" + "="*80)
    print("CREATING GCN MODEL" if not resume_checkpoint else "CREATING GCN MODEL (will load from checkpoint)")
    print("="*80)
    ...
    print(f"\nModel: GCN Baseline (Graph Convolutional Network)")
    ...
    print(f"  GCN layers: {args.gcn_num_layers} × GCN (dim={args.gcn_hidden_dim})")
    print(f"    → Standard graph convolution (Kipf & Welling 2017)")
    print(f"    → No attention mechanism (simpler than GAT)")
    print(f"  Ethnicity embedding: {n_ethnicities} → {args.ethnicity_embed_dim}")
    print(f"  Predictor: {args.gcn_hidden_dim + args.ethnicity_embed_dim} → 1")
