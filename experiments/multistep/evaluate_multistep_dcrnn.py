"""
evaluate_multistep_dcrnn.py

Multi-step prediction evaluation for DCRNN baseline starting from 2001:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Multi-step prediction evaluation for DCRNN from 2001'
    )
    ...
    # Model checkpoint
    parser.add_argument('--checkpoint_dir', type=str, default='../../checkpoints/dcrnn_baseline',
    ...
    # Output
    parser.add_argument('--output_dir', type=str, default='./evaluation_multistep_dcrnn',
...
@torch.no_grad()
def predict_one_step_dcrnn(model, population_t, census_features, ethnicity_map, device):
    """
    Run one step of DCRNN prediction (5 years forward).

    The DCRNN processes the full graph and predicts for all node-ethnicity pairs.
    """
    model.eval()
    ...
    # Create batch for all node-ethnicity pairs
    # DCRNN expects: full_graph_population_t, full_graph_census, node_idx, ethnicity, pop_t
    ...
    for eth_idx in range(n_ethnicities):
        # Create node indices for this ethnicity
        ...
        for start_idx in range(0, n_nodes, batch_size):
            ...
            # Get current population for these nodes
            batch_pop_t = population_t[batch_node_idx, eth_idx]
            ...
            # Forward pass
            output = model(batch, None, debug_batch=False)
...
def evaluate_multistep(model, populations, all_data, graph, device, output_dir):
    ...
    for config in period_configs:
        ...
        predicted_pop = predict_one_step_dcrnn(
...
def main():
    ...
    # Print summary table
    print("\n" + "="*80)
    print("MULTI-STEP PREDICTION SUMMARY - DCRNN (Starting from 2001)")
    print("="*80)
