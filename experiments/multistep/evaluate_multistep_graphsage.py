"""
evaluate_multistep_graphsage.py

Multi-step prediction evaluation for GraphSAGE baseline starting from 2001:
...
from model_graphsage_baseline import create_graphsage_model
from dataloader_with_city_filter import ReactionDiffusionDatasetFiltered


def parse_args():
    parser = argparse.ArgumentParser(
        description='Multi-step prediction evaluation for GraphSAGE from 2001'
    )
    ...
    # Model checkpoint
    parser.add_argument('--checkpoint_dir', type=str, default='../../checkpoints/graphsage_baseline',
    ...
    parser.add_argument('--sage_hidden_dim', type=int, default=128,
                        help='GraphSAGE hidden dimension')
    parser.add_argument('--sage_num_layers', type=int, default=3,
    ...
    # Output
    parser.add_argument('--output_dir', type=str, default='./evaluation_multistep_graphsage',
...
@torch.no_grad()
def predict_one_step_graphsage(model, population_t, census_features, ethnicity_map, device):
    """
    Run one step of GraphSAGE prediction (5 years forward).
    """
    model.eval()
...
def evaluate_multistep(model, populations, all_data, graph, device, output_dir):
    ...
    for config in period_configs:
        ...
        predicted_pop = predict_one_step_graphsage(
...
def main():
    ...
    model = create_graphsage_model(
        ...
        sage_hidden_dim=args.sage_hidden_dim,
    ...
    # Print summary table
    print("\n" + "="*80)
    print("MULTI-STEP PREDICTION SUMMARY - GraphSAGE (Starting from 2001)")
    print("="*80)
