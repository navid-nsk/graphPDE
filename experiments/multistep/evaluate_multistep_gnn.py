"""
evaluate_multistep_gnn.py

Multi-step prediction evaluation for GNN (GAT) baseline starting from 2001:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Multi-step prediction evaluation for GNN from 2001'
    )
    ...
    # Model checkpoint
    parser.add_argument('--checkpoint_dir', type=str, default='../../checkpoints/gnn_baseline',
    ...
    # Output
    parser.add_argument('--output_dir', type=str, default='./evaluation_multistep_gnn',
...
@torch.no_grad()
def predict_one_step_gnn(model, population_t, census_features, ethnicity_map, device):
    """
    Run one step of GNN prediction (5 years forward).
    """
    model.eval()
    ...
    for eth_idx in range(n_ethnicities):
        ...
        for start_idx in range(0, n_nodes, batch_size):
            ...
            # GNN model needs 'features' for batch_size determination
            # Create dummy features tensor with correct batch size
            dummy_features = torch.zeros(batch_size_actual, 1, device=device)

            # Create batch dictionary matching GNN's expected format
            batch = {
                'full_graph_population_t': population_t,
                'full_graph_census': census_features,
                'node_idx': batch_node_idx,
                'ethnicity': batch_eth_idx,
                'features': dummy_features  # GNN uses this for batch_size only
            }
...
def evaluate_multistep(model, populations, all_data, graph, device, output_dir):
    ...
    for config in period_configs:
        ...
        predicted_pop = predict_one_step_gnn(
...
def main():
    ...
    # Print summary table
    print("\n" + "="*80)
    print("MULTI-STEP PREDICTION SUMMARY - GNN (Starting from 2001)")
    print("="*80)
