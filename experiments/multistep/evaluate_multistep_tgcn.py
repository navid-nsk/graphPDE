"""
evaluate_multistep_tgcn.py

Multi-step prediction evaluation for TGCN baseline starting from 2001:
...
TGCN combines GCN spatial layers with GRU temporal layers.
"""
...
from model_tgcn_baseline import create_tgcn_model
from dataloader_with_city_filter import ReactionDiffusionDatasetFiltered


def parse_args():
    parser = argparse.ArgumentParser(
        description='Multi-step prediction evaluation for TGCN from 2001'
    )
    ...
    # Model checkpoint
    parser.add_argument('--checkpoint_dir', type=str, default='../../checkpoints/tgcn_baseline',
    ...
    parser.add_argument('--tgcn_hidden_dim', type=int, default=128,
                        help='TGCN hidden dimension')
    parser.add_argument('--tgcn_num_layers', type=int, default=2,
                        help='Number of TGCN layers')
    ...
    # Output
    parser.add_argument('--output_dir', type=str, default='./evaluation_multistep_tgcn',
...
@torch.no_grad()
def predict_one_step_tgcn(model, population_t, census_features, ethnicity_map, device):
    """
    Run one step of TGCN prediction (5 years forward).
    """
    model.eval()
    ...
    for eth_idx in range(n_ethnicities):
        ...
        for start_idx in range(0, n_nodes, batch_size):
            ...
            # Create batch dictionary matching TGCN's expected format
            batch = {
                'full_graph_population_t': population_t,
                'full_graph_census': census_features,
                'node_idx': batch_node_idx,
                'ethnicity': batch_eth_idx,
                'features': None  # TGCN uses full_graph_* directly
            }
...
def evaluate_multistep(model, populations, all_data, graph, device, output_dir):
    ...
    for config in period_configs:
        ...
        predicted_pop = predict_one_step_tgcn(
...
def main():
    ...
    # Create model
    print("\n" + "="*80)
    print("LOADING TGCN MODEL")
    print("="*80)

    model = create_tgcn_model(
        n_census_features=n_census_features,
        n_ethnicities=n_ethnicities,
        adjacency=graph['adjacency'],
        feature_encoder_dim=args.feature_encoder_dim,
        tgcn_hidden_dim=args.tgcn_hidden_dim,
        tgcn_num_layers=args.tgcn_num_layers,
    ...
    # Print summary table
    print("\n" + "="*80)
    print("MULTI-STEP PREDICTION SUMMARY - TGCN (Starting from 2001)")
    print("="*80)
